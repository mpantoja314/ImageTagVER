...
class Gui():
	...
	buttonDeleteStart=tk.Button(frameM, width=25,text="Delete Start (One)", command=self.deleteStart).grid(row=7,column=3,sticky="n")
	buttonDeleteStart=tk.Button(frameM, width=25,text="Delete Stop", command=self.deleteStop).grid(row=8,column=3,sticky="n")
	...
	opts = {}
        ...
        self.path = tkFileDialog.askopenfilename(title='Input File',**opts)
        if self.path == '': return
	...
	path1=tkFileDialog.askdirectory(title='Enter Output Folder') #"/home/david/Pictures"
	...
	self.path_out = tkFileDialog.askdirectory(title='Choose Output Folder')
