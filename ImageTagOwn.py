...
class Gui():
	...
	#list for names of images in a folder
	self.ImageList=[]
	...
	buttonDeleteAll=tk.Button(frameM, width=25,text="Delete ALL Square/Label", command=self.delete_all).grid(row=11,column=3,sticky="n")
	#add the save Image/ Exit
	button = tk.Button(frameM, text="Save & Exit", command=self.save).grid(row=20,column=3,pady=20,sticky="n")
